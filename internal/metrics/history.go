...
// every request.
func (h *History) Record(ctx context.Context, t Totals, storageBytes int64) error {
	q := fmt.Sprintf(`INSERT INTO metrics_snapshots (http_requests, login_success, login_failure, rate_limit_rejections, storage_bytes)
VALUES (%s, %s, %s, %s, %s)`, h.db.Placeholder(1), h.db.Placeholder(2), h.db.Placeholder(3), h.db.Placeholder(4), h.db.Placeholder(5))
	_, err := h.db.ExecContext(ctx, q, t.HTTPRequests, t.LoginSuccess, t.LoginFailure, t.RateLimitRejections, storageBytes)
	return err
}
...
func (h *History) Since(ctx context.Context, since time.Time) ([]Snapshot, error) {
	q := fmt.Sprintf(`SELECT "timestamp", http_requests, login_success, login_failure, rate_limit_rejections, storage_bytes
...
FROM metrics_snapshots WHERE [timestamp] >= %s ORDER BY [timestamp] ASC`, h.db.Placeholder(1))
	...
	for rows.Next() {
		var s Snapshot
		if err := rows.Scan(&s.Timestamp, &s.HTTPRequests, &s.LoginSuccess, &s.LoginFailure, &s.RateLimitRejections, &s.StorageBytes); err != nil {
			return nil, err
		}
		out = append(out, s)
	}
