      ...
      {settings && (
        <>
            ...
            <TableWrap>
              <table className="w-full border-collapse">
                <tbody>
                  ...
                  <Row label="Reference time zone" value={settings.timezone} />
                </tbody>
              </table>
            </TableWrap>
            <p className="text-muted text-xs mt-2">
              Timestamps throughout the app are shown in each viewer&apos;s own browser time zone
              automatically, so staff in different offices each see their own local time. The
              reference zone above is IdpForge&apos;s own (for schedules and server-side logs), not
              something applied to every timestamp.
            </p>
          </Panel>
          ...
          <p className="text-muted text-sm mb-2">
            Assigned to every new account and to any account an admin resets. Visible here only
            (admin back office) and via server config (IDPFORGE_DEFAULT_PASSWORD) — never through
            any other API. Every account carrying it is forced to change it on next login.
          </p>
          <Panel>
            <TableWrap>
              <table className="w-full border-collapse">
                <tbody>
                  <Row label="Default password" value={settings.default_password || "(not configured)"} />
                  <Row label="Minimum length" value={`${settings.password_policy.min_length} characters`} />
                  <Row label="Requires uppercase" value={String(settings.password_policy.require_uppercase)} />
                  <Row label="Requires lowercase" value={String(settings.password_policy.require_lowercase)} />
                  <Row label="Requires number" value={String(settings.password_policy.require_number)} />
                  <Row label="Requires special character" value={String(settings.password_policy.require_special)} />
                </tbody>
              </table>
            </TableWrap>
          </Panel>
